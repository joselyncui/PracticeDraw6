    ...
    protected void onAttachedToWindow() {
        ...
        animateBt.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                switch (value){
                    case 0:
                        imageView.animate().translationX(Utils.dpToPixel(100)).setDuration(300).start();
                        break;
                    case 1:
                        imageView.animate().translationX(0).start();
                        break;
                    case 2:
                        imageView.animate().translationY(Utils.dpToPixel(50)).setDuration(300).start();
                        break;
                    case 3:
                        imageView.animate().translationY(0).setDuration(300).start();
                        break;
                    case 4:
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            imageView.animate().translationZ(Utils.dpToPixel(15)).start();
                        }
                        break;
                    case 5:
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                            imageView.animate().translationZ(Utils.dpToPixel(0)).start();
                        }
                        break;
